...
namespace TheLeftExit.Memory {
    public unsafe abstract class MemorySource {
        ...
        public bool TryRead(ulong address, int count, void* buffer) => TryReadCore(address, count, buffer);
        public bool TryWrite(ulong address, int count, void* buffer) => TryWriteCore(address, count, buffer);
